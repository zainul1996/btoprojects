...
export const metadata: Metadata = {
  ...
  title: "All BTO and SBF projects | BTOProjects.sg",
  description:
    "Every HDB BTO project and SBF balance-flat pool we track, grouped by sales exercise: official prices, flat mix, waiting times and a full source trail on every page.",
  alternates: { canonical: "/projects" },
  openGraph: {
    title: "All BTO and SBF projects | BTOProjects.sg",
    description:
      "Every HDB BTO project and SBF balance-flat pool we track, grouped by sales exercise, with provenance on every fact.",
    url: "/projects",
    type: "website",
  },
};
...
export default async function ProjectsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-16 md:px-6">
      <PageHeader
        title="All projects"
        lede="Every BTO launch and SBF balance-flat pool we track, grouped by sales exercise. Each page carries its official facts, estimates and a full source trail."
      />
      ...
      {sections.map(({ exercise, projects }) => (
        <Section
          ...
          description={
            exercise.type === "sbf"
              ? `${projects.length} town pool${projects.length === 1 ? "" : "s"} in this exercise.`
              : `${projects.length} project${projects.length === 1 ? "" : "s"} in this exercise.`
          }
        >
          <div className="flex items-center gap-3">
            ...
            <Link
              href={exercise.type === "sbf" ? `/sbf/${exercise.key}` : `/bto/${exercise.key}`}
